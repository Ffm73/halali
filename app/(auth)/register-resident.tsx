...
import { Currency, Language, Gender } from '@/types';
import { 
  ArrowLeft, 
  ArrowRight, 
  User, 
...
export default function RegisterResidentScreen() {
    ...
    if (currentStep === 'invitation') {
      ...
      if (!trimmedCode) {
        Alert.alert(
          ...
          language === 'ar' ? 'يرجى إدخال رمز الدعوة' : 'Please enter invitation code'
        );
        return;
      }
      ...
      try {
        // For demo purposes, accept any of these codes
        const validDemoCodes = ['RENT2024001', 'RENT2024002', 'RENT2024003', 'DEMO001', 'DEMO002', 'DEMO003'];
        
        if (validDemoCodes.includes(trimmedCode)) {
          ...
          const mockInvitation = {
            ...
            phoneNumber: personalInfo.phoneE164 || '+966501234567',
            status: 'pending',
          ...
          setValidatedInvitation(mockInvitation);
          setCurrentStep('personal');
          console.log('✅ Demo invitation code validated:', trimmedCode);
        } else {
          // Try the actual validation function as fallback
          const invitation = await validateInvitationCode(trimmedCode);
        
          if (invitation) {
            setValidatedInvitation(invitation);
            setCurrentStep('personal');
            console.log('✅ Invitation code validated:', trimmedCode);
          } else {
            Alert.alert(
              language === 'ar' ? 'رمز غير صحيح' : 'Invalid Code',
              language === 'ar' ? 'رمز الدعوة غير صحيح أو منتهي الصلاحية. جرب أحد الرموز التجريبية.' : 'Invitation code is invalid or expired. Try one of the demo codes.'
            );
          }
        }
      } catch (error) {
        ...
        Alert.alert(
          language === 'ar' ? 'خطأ' : 'Error',
          language === 'ar' ? 'فشل في التحقق من رمز الدعوة' : 'Failed to validate invitation code'
        );
      } finally {
      ...
      if (!personalInfo.nationalId.trim()) {
        Alert.alert(
          language === 'ar' ? 'خطأ' : 'Error',
          language === 'ar' ? 'رقم الهوية مطلوب للمستأجرين' : 'National ID is required for residents'
        );
        return;
      }
  ...
  const handleRegister = async () => {
    ...
    try {
      console.log('🏠 RESIDENT REGISTRATION: Starting registration process...');
      ...
      if (result.success) {
        console.log('✅ RESIDENT REGISTRATION SUCCESSFUL - Should redirect to resident portal');
        Alert.alert(
          ...
          [
            {
              ...
              onPress: () => {
                console.log('🎯 RESIDENT: Navigating to resident portal');
        ...
        console.log('❌ RESIDENT REGISTRATION FAILED:', result.error);
        Alert.alert(
  ...
  const renderInvitationStep = () => (
    <View style={styles.stepContainer}>
      <View style={[styles.stepHeader, { alignItems: isRTL ? 'flex-end' : 'flex-start' }]}>
        ...
        >
          {language === 'ar' ? 'رمز دعوة المستأجر' : 'Tenant Invitation Code'}
        </Text>
        <Text
          style={[
        ...
        <View style={[styles.demoInvitations, { 
          ...
          <View style={styles.demoCodesList}>
            <TouchableOpacity 
              ...
              onPress={() => {
                setInvitationCode('RENT2024001');
                console.log('Demo code selected: RENT2024001');
              }}
            >
              <Text style={[styles.demoCodeItem, { 
                ...
                RENT2024001 - {language === 'ar' ? 'برج العلامة - الوحدة A-101' : 'Al-Alamah Tower - Unit A-101'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity 
              style={styles.demoCodeButton}
              onPress={() => {
                setInvitationCode('RENT2024002');
                console.log('Demo code selected: RENT2024002');
              }}
            >
              <Text style={[styles.demoCodeItem, { 
                color: colors.success,
                ...
                RENT2024002 - {language === 'ar' ? 'مجمع النور - الوحدة B-205' : 'Al-Noor Complex - Unit B-205'}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity 
              style={styles.demoCodeButton}
              onPress={() => {
                setInvitationCode('DEMO001');
                console.log('Demo code selected: DEMO001');
              }}
            >
              <Text style={[styles.demoCodeItem, { 
                color: colors.success,
                ...
                DEMO001 - {language === 'ar' ? 'رمز تجريبي عام' : 'General demo code'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <View style={styles.inputGroup}>
        ...
        <View style={styles.inputWrapper}>
          ...
          <TextInput
            ...
            placeholder="Mohammed Ahmed Alsaeed"
            placeholderTextColor={colors.textMuted}
      ...
      >
        ...
        >
          ...
          <View style={[styles.header, { alignItems: isRTL ? 'flex-end' : 'flex-start' }]}>
            ...
            >
              {language === 'ar' ? 'حساب مستأجر' : 'Tenant Account'}
            </Text>
          </View>
